...
bool java_lang_String::test_and_set_flag(oop java_string, uint8_t flag_mask)
{
  ...
  while ((value & flag_mask) == 0)
  {
    ...
    if (value == old_value)
      return false; // Flag bit changed from 0 to 1.
  }
...
struct BacktraceElement : public StackObj
{
  ...
  Symbol *_name;
  Handle _mirror;
  BacktraceElement(Handle mirror, int mid, int version, int bci, Symbol *name) : _method_id(mid), _bci(bci), _version(version), _name(name), _mirror(mirror) {}
};
...
oop java_lang_invoke_ResolvedMethodName::find_resolved_method(const methodHandle &m, TRAPS)
{
  ...
  if (method->is_old())
  {
    method = (method->is_deleted()) ? Universe::throw_no_such_method_error() : method->get_new_method();
  }
...
#define CHECK_LONG_OFFSET(klass_name, cpp_klass_name, field_name, field_sig) \
  ...
  if (!valid)
    vm_exit_during_initialization("Field offset verification failed");
}
