...
void outputStream::printContent(uintptr_t ptr, size_t size)
{
  for (size_t i = 0; i < size; ++i)
  {
    log_info(gc, task)("content%ld(off-%ld): %lx", i, i * 8, *(uint64_t *)(ptr + i * 8));
  }
}
...
{
  char *copy = c_heap ? NEW_C_HEAP_ARRAY(char, _written + 1, mtInternal) : NEW_RESOURCE_ARRAY(char, _written + 1);
...
void defaultStream::start_log()
{
  ...
  xs->head("vm_version");
  xs->head("name");
  xs->text("%s", VM_Version::vm_name());
  xs->cr();
  xs->tail("name");
  xs->head("release");
  xs->text("%s", VM_Version::vm_release());
  xs->cr();
  xs->tail("release");
  xs->head("info");
  xs->text("%s", VM_Version::internal_vm_info_string());
  xs->cr();
  xs->tail("info");
  xs->tail("vm_version");
  ...
  {
    xs->head("command");
    xs->text()->print_cr("%s", Arguments::java_command());
    xs->tail("command");
  }
  if (Arguments::sun_java_launcher() != NULL)
  {
    xs->head("launcher");
    xs->text()->print_cr("%s", Arguments::sun_java_launcher());
    xs->tail("launcher");
  }
...
intx ttyLocker::hold_tty()
{
  if (defaultStream::instance == NULL)
    return defaultStream::NO_WRITER;
  intx thread_id = os::current_thread_id();
...
{
  if (holder == defaultStream::NO_WRITER)
    return;
