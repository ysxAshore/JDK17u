...
protected:
  ...
  GrowableArrayBase(int initial_max, int initial_len) : _len(initial_len),
                                                        _max(initial_max)
  {
...
template <typename E, typename UnaryPredicate>
class GrowableArrayFilterIterator;
...
protected:
  ...
  GrowableArrayView(E *data, int initial_max, int initial_len) : GrowableArrayBase(initial_max, initial_len), _data(data) {}
...
public:
  ...
  int find_sorted(const K &key, bool &found)
  {
    found = false;
    int min = 0;
    int max = length() - 1;
...
protected:
  GrowableArrayWithAllocator(E *data, int initial_max) : GrowableArrayView<E>(data, initial_max, 0)
  {
  ...
  GrowableArrayWithAllocator(E *data, int initial_max, int initial_len, const E &filler) : GrowableArrayView<E>(data, initial_max, initial_len)
  {
    int i = 0;
    for (; i < initial_len; i++)
    {
...
public:
  GrowableArrayMetadata(Arena *arena) : _bits(bits(arena))
                                            debug_only(COMMA _nesting_check(on_stack()))
  {
  }

  GrowableArrayMetadata(MEMFLAGS memflags) : _bits(bits(memflags))
                                                 debug_only(COMMA _nesting_check(on_stack()))
  {
  }

#ifdef ASSERT
  GrowableArrayMetadata(const GrowableArrayMetadata &other) : _bits(other._bits),
...
public:
  GrowableArray(int initial_max = 2, MEMFLAGS memflags = mtNone) : GrowableArrayWithAllocator<E, GrowableArray>(
  ...
  GrowableArray(int initial_max, int initial_len, const E &filler, MEMFLAGS memflags = mtNone) : GrowableArrayWithAllocator<E, GrowableArray>(
                                                                                                     allocate(initial_max, memflags),
  ...
  GrowableArray(Arena *arena, int initial_max, int initial_len, const E &filler) : GrowableArrayWithAllocator<E, GrowableArray>(
...
public:
  GrowableArrayCHeap(int initial_max = 0) : GrowableArrayWithAllocator<E, GrowableArrayCHeap<E, F>>(
                                                allocate(initial_max, F),
                                                initial_max) {}

  GrowableArrayCHeap(int initial_max, int initial_len, const E &filler) : GrowableArrayWithAllocator<E, GrowableArrayCHeap<E, F>>(
                                                                              allocate(initial_max, F),
                                                                              initial_max, initial_len, filler) {}
...
class GrowableArrayIterator : public StackObj
{
  ...
  template <typename F, typename UnaryPredicate>
  friend class GrowableArrayFilterIterator;
...
public:
  GrowableArrayFilterIterator(const GrowableArrayIterator<E> &begin, UnaryPredicate filter_predicate) : _array(begin._array), _position(begin._position), _predicate(filter_predicate)
  {
