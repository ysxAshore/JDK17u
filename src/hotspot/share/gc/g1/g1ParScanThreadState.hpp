...
public:
  // @insert: get taskqueue info
  uintptr_t getTaskQueueElemsBase() { return _task_queue->getTaskQueueElemsBase(); }
  ...
  template <class T>
  void write_ref_field_post(T *p, oop obj);

  template <class T>
  void enqueue_card_if_tracked(G1HeapRegionAttr region_attr, T *p, oop o)
  {
