...
G1Allocator::G1Allocator(G1CollectedHeap *heap) : _g1h(heap),
                                                  _numa(heap->numa()),
                                                   ...
                                                   uint node_index)
{
  ...
  HeapWord *result = survivor_gc_alloc_region(node_index)->attempt_allocation(min_word_size, desired_word_size, actual_word_size);
  if (result == NULL && !survivor_is_full())
  {
    MutexLocker x(FreeList_lock, Mutex::_no_safepoint_check_flag);
    result = survivor_gc_alloc_region(node_index)->attempt_allocation_locked(min_word_size, desired_word_size, actual_word_size);
    if (result == NULL)
    {
      set_survivor_full();
    }
...
G1PLABAllocator::G1PLABAllocator(G1Allocator *allocator) : _g1h(G1CollectedHeap::heap()),
...
HeapWord *G1PLABAllocator::allocate_direct_or_new_plab_debug(G1HeapRegionAttr dest,
                                                             size_t word_sz,
...
HeapWord *G1ArchiveAllocator::archive_mem_allocate(size_t word_size)
{
  ...
  assert(_bottom <= old_top && old_top <= _max,
         "inconsistent allocation state: expected " PTR_FORMAT " <= " PTR_FORMAT " <= " PTR_FORMAT,
